...
const dest = path.join(packages, 'sjalq/schelm-node-runtime-api-feasibility/1.0.0');
...
fs.writeFileSync(path.join(packages, 'registry.dat'), add(registry, 'sjalq', 'schelm-node-runtime-api-feasibility'));
fs.mkdirSync(path.join(repo, 'build'), { recursive: true });
for (const mode of ['debug', 'optimize']) {
  const args = ['make', 'src/Main.elm', '--output', path.join(repo, `build/api-feasibility-${mode}.js`)];
