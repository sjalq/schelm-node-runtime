...
for(const artifact of ['debug','optimize']) for(const mode of ['release','backstop','restore-failed','share']) test(`compiled ${artifact} PTY ${mode} restores cooked`,()=>{
  const out=cp.execFileSync('python3',['tests/pty/pty-runner.py',mode,artifact],{cwd:root,encoding:'utf8',timeout:10000});
  assert.match(out,mode==='share'?/SHARE=/:/RAW/);
