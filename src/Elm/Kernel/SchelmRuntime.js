...
var _SchelmRuntime_acquireTerminal = __Scheduler_binding(function(cb){
  ...
  var lease={id:$nextLease++,raw:false,poison:false,decoder:new TextDecoder('utf-8',{fatal:false}),strictDecoder:new TextDecoder('utf-8',{fatal:true}),remainder:Buffer.alloc(0),ended:false,eofFlushed:false,eofMalformed:false}; $terminalLease=lease; var back=function(){$restore(lease.id);}; var ended=function(){lease.ended=true;}; lease.back=back;lease.endedListener=ended;process.on('exit',back);process.stdin.on('end',ended);
...
var _SchelmRuntime_read = function(id){return __Scheduler_binding(function(cb){
  ...
  function deliver(bytes){var take=bytes.subarray(0,16384);lease.remainder=bytes.subarray(take.length);var bad=malformed(take,true);if(bad)lease.eofMalformed=true;var text=lease.decoder.decode(take,{stream:true});finish({__$kind:'piece',__$text:text,__$bytes:take.length,__$malformed:bad});}
  function flush(){if(lease.eofFlushed){finish({__$kind:'end',__$malformed:lease.eofMalformed});return;}lease.eofFlushed=true;var bad=malformed(new Uint8Array(0),false),text=lease.decoder.decode();lease.eofMalformed=lease.eofMalformed||bad;lease.decoder=new TextDecoder('utf-8',{fatal:false});lease.strictDecoder=new TextDecoder('utf-8',{fatal:true});if(text||bad)finish({__$kind:'piece',__$text:text,__$bytes:0,__$malformed:bad});else finish({__$kind:'end',__$malformed:lease.eofMalformed});}
