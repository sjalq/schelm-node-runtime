...
onEffects router commands subscriptions state =
    let
        ( acquireCallbacks, otherCommands ) =
            splitAcquire commands
    in
    applyAcquire router acquireCallbacks state
        |> Task.andThen (applyCommands router otherCommands)
        |> Task.andThen (syncResize router subscriptions)


splitAcquire commands =
    let
        step command_ ( callbacks, others ) =
            case command_ of
                Acquire _ callback ->
                    ( callback :: callbacks, others )

                _ ->
                    ( callbacks, command_ :: others )

        ( callbacks, others ) =
            List.foldl step ( [], [] ) commands
    in
    ( List.reverse callbacks, List.reverse others )
    ...
    case callbacks of
        ...
        _ ->
            ...
            case state.terminal of
                Just _ ->
                    sendCallbacks router accepted (Err Busy) state
                        |> Task.andThen (sendCallbacks router rejected (Err TooManyAcquireJoiners))
                            ...
                            (\raw ->
                                case raw.kind of
                                    "ok" ->
                                        ...
                                        in
                                        sendCallbacks router accepted (Ok terminal) { state | terminal = Just terminal }
                                            |> Task.andThen (sendCallbacks router rejected (Err TooManyAcquireJoiners))

                                    "not" ->
                                        sendCallbacks router accepted (Err NotInteractive) state
                                            |> Task.andThen (sendCallbacks router rejected (Err TooManyAcquireJoiners))
                                    ...
                                    "restore" ->
                                        sendCallbacks router accepted (Err RestoreFailed) state
                                            |> Task.andThen (sendCallbacks router rejected (Err TooManyAcquireJoiners))

                                    _ ->
                                        sendCallbacks router accepted (Err AcquireFailed) state
                                            |> Task.andThen (sendCallbacks router rejected (Err TooManyAcquireJoiners))
                            )


sendCallbacks router callbacks result state =
    case callbacks of
        [] ->
            Task.succeed state

        callback :: rest ->
            send router callback result state |> Task.andThen (sendCallbacks router rest result)
